...
export class AffectationCaissierComponent implements OnInit {
  ...
  constructor(
    ...
    private usercaisseservice: AffectationCaisseService,
  ...
  async ngOnInit() {
    ...
    // Charger d'abord les affectations existantes
    await this.loadUserCaisses();
    // Puis charger les caisses (en filtrant par usedCaisses)
    await this.loadAllCaisses();
  ...
  // Méthode pour charger les affectations
  async loadUserCaisses(): Promise<void> {
    const params = {
    ...
    const res = await firstValueFrom(this.usercaisseservice.getAll(params));
    if (res.success) {
      this.usercaisses = res.data.data;
      this.usedCaisses = this.usercaisses
        .map((u) => u.idcaisse)
        .filter((id) => id !== null);
    }
  }

  // Méthode pour charger les caisses actives
  async loadAllCaisses(): Promise<void> {
    const allactif = { page: 1, limit: 1000, search: '', actif: 1 };
    const res = await firstValueFrom(this.caisseservice.getAll(allactif));
    if (res.success) {
      this.allCaisses = res.data.data;
      // Filtrer les caisses déjà utilisées
      this.caisses = this.allCaisses.filter(
        (caisse) => !this.usedCaisses.includes(caisse.idcaisse),
      );
    } else {
      console.error('Erreur lors du chargement des caisses :', res);
    }
  }
  ...
  async modalCreate() {
    ...
    //Initialiser le formulaire
    this.initForm();
    await this.loadUserCaisses(); // recharge les affectations
    await this.loadAllCaisses();
  }

  //Modification du modal
  async modalUpdate(item: AffectationCaisseModel) {
    this.usercaisse = item;
    this.actionModal = 'update';
    this.usercaisseForm.reset();
    await this.loadUserCaisses(); // important pour mettre à jour usedCaisses
    await this.loadAllCaisses();
    // Ajouter la caisse de l'affectation en cours (pour qu'elle soit dans la liste)
    if (this.usercaisse.caisse) {
  ...
  deleteConfirmed() {
    ...
    this.usercaisseservice
      .delete(this.deleteElement.idutilisateurcaisse)
      .subscribe({
        next: (res) => {
          if (res.success) {
  ...
  onSubmit() {
    ...
    const _affectation: any = {
      ...formValue,
      createdby: this.userConnect.nom + ' ' + this.userConnect.prenom,
    };
  ...
  saveLigne() {
    ...
    const _affectation: any = {
      ...ligne,
      actif: ligne.actif ? 1 : 0,
      createdby: this.userConnect.codeutilisateur ?? null,
      updatedby: this.userConnect.codeutilisateur ?? null,
    };
    // Appel API
