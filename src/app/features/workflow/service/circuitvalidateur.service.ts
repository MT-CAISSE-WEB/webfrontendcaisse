import { HttpClient } from "@angular/common/http";
import { Observable } from "rxjs";
import { QueryResultModel } from "../../../_core/models/query-result.model";
import { URL_LOCAL } from "../../../_core/routes/backend.root";
import { Injectable } from "@angular/core";
import { circuitvalidateurmodel } from "../model/circuitvalidateur.model";

@Injectable({
  providedIn: 'root'
})
export class circuitvalidateurservice {
    url : string = 'circuitvalidateur' ;

    constructor(private http: HttpClient) {}

    /**
   * get All
   * @param params
   */
    getAll(params: any = {}): Observable<QueryResultModel> {
        return this.http.get<QueryResultModel>(URL_LOCAL.baseUrl + this.url, {
        params,
        });
    }

  /**
   *upsert
   * @param _object
   * 
   */
   create (_object: any): Observable<any> {
    return this.http.post<any>(
      URL_LOCAL.baseUrl + this.url,
      _object
    );
  }

    /**
   *upsert
   * @param _object
   * 
   */
   update (_object : circuitvalidateurmodel): Observable<circuitvalidateurmodel> {
    return this.http.put <circuitvalidateurmodel>(
      URL_LOCAL.baseUrl + this.url+"/"+_object.idcircuitvalidateur,_object
    );
  }

  /**
   * delete
   * @param id
   */
  delete(id: string): Observable<any> {
    return this.http.delete<any>(
      URL_LOCAL.baseUrl + this.url + "/" + id
    );
  }


  /**
   * get one
   * @param id
   */
  getOne(id: string): Observable<any> {
    return this.http.get<any>(
      URL_LOCAL.baseUrl + this.url + "/" + id
    );
  }

}