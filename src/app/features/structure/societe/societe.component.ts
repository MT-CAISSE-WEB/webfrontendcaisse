...
export class SocieteComponent {
  ...
  searchsociete() {
    ...
    this.filtresocietes = this.societes.filter((societe) => {
      ...
      const matchtext =
        codesociete.includes(term) || raisonsociale.includes(term);
  ...
  get actifCount(): number {
    return this.societes.filter((societe) => societe.suivibudgetaire === 1)
      .length;
  }

  get inactifCount(): number {
    return this.societes.filter((societe) => societe.suivibudgetaire === 0)
      .length;
  }
  ...
  onSubmit() {
    ...
    if (this.societeForm.invalid) {
      Object.values(this.societeForm.controls).forEach((c) =>
        c.markAsTouched(),
      );
