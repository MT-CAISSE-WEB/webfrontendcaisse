...
@Component({
  ...
  template: `<div *ngFor="let option of chartCaisseOptions" class="mb-5">
    <div echarts [options]="option" class="h-800"></div>
  </div>`,
})
export class OperationCaisseChartComponent implements OnInit {
  ...
  loadStatsMouvementsCaisse() {
    this.loading = true;
    this.service.getMouvementsCaisse(this.idsociete, this.idsite).subscribe({
      next: (res: any) => {
        if (res.success) {
          this.loading = false;
          ...
          this.chartCaisseOptions = Object.keys(groupedByCaisse).map(
            (key: any) => {
              ...
              caisse.data.sort(
                (a: any, b: any) =>
                  new Date(a.jourRaw).getTime() - new Date(b.jourRaw).getTime(),
              );
